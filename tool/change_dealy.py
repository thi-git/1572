...
def wash():
    ...
    # 需要清洗的檔案存放在old_file
    # Step1. 取得所有待清洗的檔案(convert不處理/下載檔的做清洗/上傳檔的複製原檔)
    ...
    for each_folder in all_folders:
        ...
        for file in all_files:
            ...
            if 'convert' not in file:
                ...
                if '下載檔' in file or '鼎漢' in file:
                    ...
                    # 取得分頁三PCE資料
                    ws_p3 = wb_org['平日晨峰(0630-0830)', '平日晨峰(07-09)', '平日中午(11-13)', '平日中午(12-14)', '平日昏峰(17-19)',
        '假日(11-12)', '假日(1430-1530)', '假日(1730-1830)', '假日(16-18)', '假日(17-19)', '假日(16-20)']
                    ...
                    # 3. 取得對應的下載模板
                    fn = f"{CFG.DOWNLOAD_FOLDER}/delay/{intersection_type}_{is_holiday}.xlsx"
                    wb = load_workbook(fn)
                    sheet_list = ['資料儲存', '基本資料']
                    ...
                    # 第三頁資料
                    ws_new = wb['平日晨峰(0630-0830)', '平日晨峰(07-09)', '平日中午(11-13)', '平日中午(12-14)', '平日昏峰(17-19)',
        '假日(11-12)', '假日(1430-1530)', '假日(1730-1830)', '假日(16-18)', '假日(17-19)', '假日(16-20)']
                    for col_num, value in enumerate(pce_data, start=1):
                        ws_new.cell(row=1, column=col_num, value=value)

                    # 存檔
                    save_path = f"export/delay/{tc_id}"
                    file_name = f"{tc_id}_{date}_delay_下載檔.xlsx"
                ...
                if '上傳檔' in file:
                    ...
                    # 新檔案路徑
                    destination_file = f"{CFG.EXPORT_FOLDER}/delay/{each_folder}/{file}"

                    # 檢查目錄是否存在，若不存在則創建目錄
                    if not os.path.isdir(f"{CFG.EXPORT_FOLDER}/delay/{each_folder}"):
                        os.makedirs(f"{CFG.EXPORT_FOLDER}/delay/{each_folder}")
